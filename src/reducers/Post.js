...
export default function post(state = initialStatePosts, action) {
  ...
  switch (action.type) {
    ...
    case ADD_CATEGORY_POSTS:
      let posts = (
        newState.posts.filter(pst => pst.category !== action.categoryName)
      )
    ...
    case CREATE_POST:
      ...
      return newState
    case ADD_VOTE_POST:
      newState.posts.find(pst => pst.id === action.id).voteScore += 1
    ...
    case REMOVE_VOTE_POST:
      ...
      return newState
    case UPDATE_POST:
      const post = state.posts.find(pst => pst.id === action.post.id)
      post.body = action.post.body
      post.title = action.post.title

      return newState
    case REPLACE_POST:
      newState.posts = newState.posts.filter(pst => pst.id !== action.post.id)
      newState.posts.push(action.post)
